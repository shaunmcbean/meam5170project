...
def create_kitchen(w=0.7, h=.7):
    ...
    #mass = 0.01
    #mass = 1e-6
    cabbage = create_box(.07, .07, .1, mass=mass, color=(0, 1, 0, 1)) # green
    #cabbage = load_model(BLOCK_URDF, fixed_base=False)
    set_point(cabbage, (2, -0.2, h + .1/2))

    cheese = create_box(.07, .07, .1, mass=mass, color=(0.9, 0.9, 0.15, 1)) # yellow
    set_point(cheese, (2, 0.2, h + .1/2))

    salt = create_box(.07, .07, .1, mass=mass, color=(1, 1, 1, 1)) # white
